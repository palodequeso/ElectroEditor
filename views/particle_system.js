...
const Timer = require('exo').Timer;

const Electro = require('electroengine');
const GraphicsSystem = Electro.models.systems.Graphics;
const Entity = Electro.models.ecs.Entity;
const ParticleSystemInstance = Electro.models.particle_systems.ParticleSystemInstance;
const ParticleSystemComponent = Electro.models.components.ParticleSystem;
const Game = Electro.models.Game;
const Renderer = Electro.views.Renderer;
...
const edit_particle_system_tmpl = fs.readFileSync(path.join(__dirname, '/../tmpl/particle_system_editor.html'), 'utf8');
...
class ParticleSystemEditor extends View {
    ...
    init_test_game() {
        const test_path = path.relative(path.join(__dirname, '../../'), this.game.path);
    ...
    get_values() {
        const start_color_begin = this.element.querySelector('#start_color_range_begin').value.split(',');
        const start_color_end = this.element.querySelector('#start_color_range_end').value.split(',');
        const end_color_begin = this.element.querySelector('#end_color_range_begin').value.split(',');
        const end_color_end = this.element.querySelector('#end_color_range_end').value.split(',');
    ...
    save(event) {
        ...
        const new_id = values.id;
        this.model.set(values);

        const current_path = path.normalize(path.join(this.game.path, 'particle_systems',
                                                    this.model.id.toString() + '.json'));
        const new_path = path.normalize(path.join(this.game.path, 'particle_systems',
                                                new_id + '.json'));
    ...
    run() {
        const current_time = this.timer.milliseconds();
